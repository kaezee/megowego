  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.base }}>
      ...
      <div style={{ padding: '48px 20px 16px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: `2px solid ${C.grey100}` }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 20, color: C.ink }}>chai @ irani</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {stage === 'plan' && <>
          <HeaderCard color={C.blue}>
            ...
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 36, marginTop: 4 }}>{mono('23:41:09')}</div>
          ...
          <div>
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 16, color: C.ink, marginBottom: 10 }}>they're in ({yes.length})</div>
            {yes.map(p => <Row key={p.name} person={p} badge="✓" badgeColor={C.green} />)}
          </div>
          <div>
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 16, color: C.ink, marginBottom: 10 }}>they're out ({no.length})</div>
            {no.map(p => <Row key={p.name} person={p} badge="✗" badgeColor={C.error} />)}
          </div>
          <div>
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 16, color: C.ink, marginBottom: 10 }}>still ghosting you ({ghost.length + maybe.length})</div>
        ...
        {stage === 'commit' && <>
          <HeaderCard color={C.yellow}>
            ...
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 28, color: C.ink, marginTop: 4 }}>clear skies ☀️</div>
          ...
          <div>
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 16, color: C.ink, marginBottom: 10 }}>confirmed squad</div>
        ...
        {stage === 'happen' && <>
          <HeaderCard color={C.green}>
            ...
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 28, color: C.ink, marginTop: 4 }}>who made it?</div>
        ...
        {stage === 'settle' && <>
          <Card padding={20}>
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 18, color: C.ink, marginBottom: 16 }}>who paid?</div>
            {yes.slice(0, 2).map((p, i) => (
              <div key={p.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 12, marginBottom: i < 1 ? 12 : 0, borderBottom: i < 1 ? `1px solid ${C.grey100}` : 'none' }}>
                ...
                <div style={{ fontFamily: "'Space Mono', ui-monospace, monospace", fontWeight: 700, fontSize: 16, color: C.ink }}>₹{i === 0 ? '840' : '360'}</div>
              </div>
            ))}
          </Card>
          <Card padding={20}>
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 18, color: C.ink, marginBottom: 16 }}>everyone owes</div>
            {yes.map(p => (
              <div key={p.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <Chip color={C.grey100} style={{ fontFamily: "'Space Mono', ui-monospace, monospace", fontWeight: 700 }}>₹300</Chip>
              </div>
            ))}
          </Card>
        </>}
        ...
        {stage === 'remember' && <>
          <HeaderCard color={C.purple}>
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 28 }}>that's a wrap 🫡</div>
          ...
          <div>
            <div style={{ fontFamily: "'Fredoka', system-ui, sans-serif", fontWeight: 700, fontSize: 16, color: C.ink, marginBottom: 12 }}>how was it?</div>
...
function Row({ person, badge, badgeColor, action }: { person: { name: string; color: string }, badge: string, badgeColor: string, action?: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingBottom: 12 }}>
      ...
      {action || <span style={{ fontFamily: "'Space Mono', ui-monospace, monospace", fontWeight: 700, fontSize: 14, color: badgeColor }}>{badge}</span>}
    </div>
  )
}
